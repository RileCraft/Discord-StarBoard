    ...
    static cooldown(command, message, isInt, whInt) {
        if (command.cooldown) {
            ...
            if (isInt && whInt) {
                ...
                let date = Date.now()
                let data = await db.get(`${id}.${command.name}.${whInt}.cooldown`)
                ...
                if (Math.floor(date - data) >= time || !data) {
                    await db.set(`${id}.${command.name}.${whInt}.cooldown`, date)
                    return true
                } else return false
            } else {
                let time = command.cooldown
                ...
                let date = Date.now()
                let data = await db.get(`${id}.${command.name}.cooldown`)
                coolTime.push(Math.floor(Math.floor(data + time) / 1000))
                if (isNaN(time)) throw new Error("Invalid number in cooldown provided at " + command.name)
                if (Math.floor(date - data) >= time || !data) {
                    await db.set(`${id}.${command.name}.cooldown`, date)
                    return true
                } else return false
            }
            })()
